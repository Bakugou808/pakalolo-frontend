...
import { headers } from "../../Redux/actions/entriesActions";
...
function EntriesTable(props) {
  ...
  const {
    ...
    collectionEntries,
    selectedSmokeList,
    onPostSmokeListEntry,
    onSetEntriesForSmokeList,
    selectedEntriesForSmokeList,
    onDeleteSmokeListEntry,
    totalCollection,
    subEntryTable,
    allEntries,
    selectedEntry,
    pageEndPoint,
    snackBarEntrySuccessDisplay,
    onCloseSnackBar,
    allSmokeLists,
    onFetchSmokeLists,
    tagEntries,
  ...
  const handleClick = (event, index, row) => {
    ...
    if (selectedEntries.includes(row)) {
      newSelectedEntries = newSelectedEntries.concat(
        selectedEntries.slice(0, selectedEntries.indexOf(row)),
        selectedEntries.slice(selectedEntries.indexOf(row) + 1)
      );
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, entriesForStrain.length - page * rowsPerPage);
  ...
  return (
    <div className={classes.root}>
        ...
        <Grow in={grow}>
          <Paper className={classes.paper}>
            <EnhancedTableToolbar
              handleAddEntryToSmokeList={handleAddEntryToSmokeList}
              entriesPage={entriesPage}
              onFetchCollection={onFetchCollection}
              smokeListPage={smokeListPage}
              numSelected={selected.length}
              handleDelete={handleDelete}
              handleEdit={handleEdit}
              setForm={setForm}
              onSetEntriesForSmokeList={onSetEntriesForSmokeList}
              onFetchEntries={onFetchEntries}
              checkSelectedSmokeList={checkSelectedSmokeList}
            />
            <TableContainer>
              <Table
              ...
              >
                ...
                <TableBody>
                  {selectedEntriesForSmokeList.length > 0 && !subEntryTable
                    ...
                    : entriesPage
                    ? allEntries && renderRows(allEntries)
                    : entriesForStrain && renderRows(entriesForStrain)}
      ...
      {subEntryTable && (
        <Snackbar
          open={snackBarEntrySuccessDisplay}
                ...
                <List>
                  {allSmokeLists.map((smokeList, index) => (
                    <ListItem
                      button
                      key={smokeList.id}
                      onClick={() => handleSelectSL(smokeList)}
                    >
                      <ListItemIcon>
                        <SpaIcon />
                      </ListItemIcon>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  onFetchEntries: (userId, smokeListPage = false) =>
    fetchEntries(userId, dispatch, smokeListPage),
  onFetchCollection: (userId) => fetchCollection(userId, dispatch),
  onPostSmokeListEntry: (data, page) =>
    postSmokeListEntry(data, dispatch, page),
  onDeleteSmokeListEntry: (data) => deleteSmokeListEntry(data, dispatch),
  onSetEntriesForSmokeList: (entries) =>
    dispatch(setEntriesForSmokeList(entries)),
...
export default AuthHOC(
  connect(mapStateToProps, mapDispatchToProps)(EntriesTable)
);

...
function MapsContainer(props) {
  ...
  const { constraints, mapsLoaded, markers, searchResults, map, mapsApi, usersLocation, autoCompleteService, placesService, geoCoderService, directionService } = state;
  ...
  // find Lat and Long of address and add Marker
  const geoCode = (value) => {
    
    geoCoderService.geocode({ address: value.address }, ((response) => {
      
      const { location } = response ? response[0].geometry : {location: false}
      location && addMarker(location.lat(), location.lng(), value.name);
    }))
  }
      ...
      {(searchResults && searchResults.length > 0) ?
        <>
          ...
          <section className={classes.nothing}>
            ...
            <div className={classes.nothing}>
              {/* "mb-4 fw-md" */}
              <h1 className={classes.nothing}>Tadah! Ice-Creams!</h1>
              {/* "d-flex flex-wrap" */}
              <div className={classes.nothing}>
